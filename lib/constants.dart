// import 'package:lendana5/encryption_util.dart';
// const String API_TOKEN =
//     '';
// final String encryptedToken = EncryptionUtil.encryptValue(API_TOKEN);
// const String BASE_URL = 'http://10.0.2.2:1337';
// const String API_TOKENP =
//     '';
// const String BASE_URLP = 'https://strap-do-r3lod.ondigitalocean.app';

import 'package:flutter_dotenv/flutter_dotenv.dart';

final String API_TOKEN = dotenv.env['API_TOKEN'] ?? '';
final String BASE_URL = dotenv.env['BASE_URL'] ?? 'http://10.0.2.2:1337';
final String API_TOKENP = dotenv.env['API_TOKENP'] ?? '';
final String BASE_URLP =
    dotenv.env['BASE_URLP'] ?? 'https://strap-do-r3lod.ondigitalocean.app';


// import 'package:flutter_dotenv/flutter_dotenv.dart';
// import 'package:lendana5/encryption_util.dart';

// final String apiToken = dotenv.env['API_TOKEN'] ?? '';
// final String encryptedToken = EncryptionUtil.encryptValue(apiToken);
// final String baseUrl = dotenv.env['BASE_URL'] ?? 'http://10.0.2.2:1337';
// final String apiTokenP = dotenv.env['API_TOKENP'] ?? '';
// final String baseUrlP = dotenv.env['BASE_URLP'] ?? 'https://strap-do-r3lod.ondigitalocean.app';
