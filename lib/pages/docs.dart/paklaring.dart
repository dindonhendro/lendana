...
class paklaring extends StatefulWidget {
  final int? userId;

  const paklaring({Key? key, required this.userId}) : super(key: key);

  @override
  _paklaringState createState() => _paklaringState();
}

class _paklaringState extends State<paklaring> {
                ...
                return Center(
                  child: ListView(
                    children: [
                      Column(
                        children: [
                          Text('Paklaring:', style: TextStyle(fontSize: 12)),
                          SizedBox(
                            width: 100,
                            height: 100,
                            child: ClipOval(
                              child: Image.network(
                                'http://10.0.2.2:1337${user.paklaring?.url ?? '/uploads/wm3_51e2930f2d.jpg'}',
                                fit: BoxFit.cover,
                              ),
                            ),
                          ),
